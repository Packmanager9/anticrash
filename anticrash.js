...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    for(let t = 0;t<12;t++){
        ...
        let ball = new Circle(center.x+(Math.cos(angle)*200), center.y+(Math.sin(angle)*200),(t*1)+3, "#00ff00")
        // ball.friction = .99
        ball.r = 0
    ...
    for(let t = 0;t<balls.length;t++){
        ...
        for(let t = 0;t<balls.length;t++){
            // balls[t].xmom += (center.x-balls[t].x)*.001
            // balls[t].ymom += (center.y-balls[t].y)*.001
            for(let k = 0;k<balls.length;k++){
                if(t!=k){
                    if(balls[t].doesPerimeterTouch(balls[k])){
                        let link = new LineOP(balls[t], balls[k])
                        let adjust =  Math.abs(link.hypotenuse())//*2//Math.abs(link.hypotenuse() - (balls[t].radius+balls[k].radius))//*2
            ...
            point.color = `rgb(${balls[t].r},${255-balls[t].r}, 0)`
            balls[t].line.unshift(point)
            balls[t].frictiveMove()
            balls[t].color = `rgb(${balls[t].r},${255-balls[t].r}, 0)`
            balls[t].draw()
