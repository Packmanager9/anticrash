...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "#000000") {
        ...
        window.addEventListener('pointerdown', e => {
            ...
            let ball = new Circle(TIP_engine.x, TIP_engine.y, 10, "white")
            ball.friction = .99
            ball.r = 0
            ball.b = 0
            // ball.xmom = Math.random()-.5
            // ball.ymom = Math.random()-.5
            ball.reflect = 1
            ball.line = []
            balls.push(ball)
    ...
    for(let t = 0;t<1200;t++){
        angle += (Math.PI/6)
        // let ball = new Circle(Math.random()*canvas.width,Math.random()*canvas.height,2+(Math.random()*12), getRandomColor() )
        let ball = new Circle(center.x+(Math.cos(angle)*200), center.y+(Math.sin(angle)*200),(t*.001)+3, "#00ff00")
        ball.friction = .99999
        ball.r = 0
        ball.b = 0
        ball.megalink = new LineOP(ball, center)
        // ball.xmom = Math.random()-.5
        // ball.ymom = Math.random()-.5
        ball.reflect = 1
        ball.line = []
    ...
    //     for(let k = 0;k<balls[t].line.length-1;k++){
    //         let link = new LineOP(balls[t].line[k], balls[t].line[k+1], balls[t].line[k].color, 1)
    //         link.draw()
    //     }
    // }
        for(let t = 0;t<balls.length;t++){
            balls[t].xmom += (center.x-balls[t].x)/100000//(balls[t].megalink.hypotenuse())
            balls[t].ymom += (center.y-balls[t].y)/100000//(balls[t].megalink.hypotenuse())
            // balls[t].a = 1.4-((balls[t].megalink.hypotenuse()) / 300)
            balls[t].a = 1
            for(let k = 0;k<balls.length;k++){
                if(t!=k){
                    if(balls[t].doesPerimeterTouch(balls[k])){
                        let link = new LineOP(balls[t], balls[k])
                        let adjust = Math.abs(link.hypotenuse() - (balls[t].radius+balls[k].radius))//*2 link.hypotenuse()//*2//
                        balls[t].x += (.5*Math.cos(link.angle())*adjust)
                        balls[t].y += (.5*Math.sin(link.angle())*adjust)
                        balls[k].x -= (.5*Math.cos(link.angle())*adjust)
                        balls[k].y -= (.5*Math.sin(link.angle())*adjust)
                        balls[t].xmom*=.99
                        balls[t].ymom*=.99
                    ...
                    // balls[t].xmom += (balls[k].x-balls[t].x)*.00001
                    // balls[t].ymom += (balls[k].y-balls[t].y)*.00001
                }
            }
            balls[t].r-=9
            if(balls[t].r < 0){
            ...
            let point = new Point(balls[t].x, balls[t].y)
            point.color = `rgba(${balls[t].r},${255-balls[t].r}, ${balls[t].b}, ${balls[t].a})`
            balls[t].line.unshift(point)
            balls[t].frictiveMove()
            balls[t].color = `rgba(${balls[t].r},${255-balls[t].r}, ${balls[t].b}, ${balls[t].a})`
            balls[t].draw()

            // let point2 = new Point(balls[t].x, balls[t].y)
            // balls[t].line.push(point2)
            if(balls[t].line.length > 100){
                balls[t].line.splice(100,10)
            }
        }
